...
class FunctorFromHaskToHask2 t where
  fmapFromHaskToHask2 :: (Functor (t a), Functor (t b)) => (a -> b) -> (t a ~> t b)
...
class FunctorFromHask2ToHask t where
  fmapFromHask2ToHask :: (Functor f, Functor g) => (f ~> g) -> (t f -> t g)
...
instance FunctorFromHaskToHask2 Const where
  fmapFromHaskToHask2 :: (a -> b) -> (Const a ~> Const b)
  fmapFromHaskToHask2 a2b = \(MkConst a) -> MkConst (a2b a)
...
instance FunctorFromHask2ToHask SomeF where
  fmapFromHask2ToHask :: (f ~> g) -> (SomeF f -> SomeF g)
  fmapFromHask2ToHask f2g = \(MkSomeF fa) -> MkSomeF (f2g fa)
...
instance FunctorFromHask2ToHask AnyF where
  fmapFromHask2ToHask :: (f ~> g) -> (AnyF f -> AnyF g)
  fmapFromHask2ToHask f2g = \(MkAnyF fa) -> MkAnyF (f2g fa)
